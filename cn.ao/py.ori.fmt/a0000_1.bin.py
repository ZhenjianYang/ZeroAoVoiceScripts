...
def main():
    CreateScenaFile(
        ...
        # Information
        [512, 924, 1648, 1649, 1771, 2495, 3165, 3795, 4012, 0, 4295, 0, 4747, 5328, 5839, 6332, 0, 6596, 0, 229, 27, 0, 0],
    )
    ...
    Menu(
        ...
        (
            ...
            "e4400 メルカバ外観(昼) \x01",            # 7
            "e3020 メルカバ内部(昼) \x01",            # 8
            "e440B メルカバ外観(夜) \x01",            # 9
            "e302B メルカバ内部(夜) \x01",            # 10
            "e2010 特務支援カー内部(昼) \x01",        # 11
            "e2110 共和国大統領用リムジン \x01",      # 12
            "e3210 アルセイユ内部 \x01",              # 13
            "e4800 共和国行き列車ループ\x01",         # 14
            "キャンセル\x01",                         # 15
        )
    )
    ...
    Menu(
        ...
        1,
        (
            "C1500 オルキスタワー外観(昼)\x01",           # 0
            "C1600 オルキスタワー屋上(昼)\x01",           # 1
            "C1510 エントランス(昼)\x01",                 # 2
            "C1530 34F警察、マスコミ控え室(昼)\x01",      # 3
            "C1540 35F大会議場(昼)\x01",                  # 4
            "C1550 36F国賓控え室(昼)\x01",                # 5
            "C1560 20F新市長室(昼)\x01",                  # 6
            "C1580 ダンジョンエリア01\x01",               # 7
            "C1581 ダンジョンエリア02\x01",               # 8
            "C1582 ダンジョンエリア03\x01",               # 9
            "C1583 Ｆ30、Ｆ40\x01",                       # 10
            "C1584 Ｆ31～Ｆ33非常階段\x01",               # 11
            "E4020 Ｆ36反転カメラ用\x01",                 # 12
            "キャンセル\x01",                             # 13
        )
    )
    ...
    Menu(
        ...
        (
            "C000B 駅前通り(夜)\x01",                     # 0
            "C110D 行政区襲撃後(昼)\x09\x01",             # 1
            "C120D 湾岸区襲撃後(昼)\x09\x01",             # 2
            "C040D 歓楽街襲撃後(昼)\x09\x01",             # 3
            "C140D 旧市街襲撃後(昼)\x09\x01",             # 4
            "C115D 警察所一階襲撃時(夜)\x09\x01",         # 5
            "C121D 黒月内部襲撃時(夜) \x01",              # 6
            "E3800 旧市街ヴァルド暴れ(夜)\x01",           # 7
            "E3900 IBC大炎上(夜)\x01",                    # 8
            "C0180 中央区デパート屋上(昼) \x01",          # 9
            "C018D 中央区デパート屋上炎上(夜) \x01",      # 10
            "C1340 IBC総裁室(昼) \x01",                   # 11
            "キャンセル\x01",                             # 12
        )
    )
    ...
    Menu(
        ...
        (
            ...
            "t100b ?????外観ホテル前(夜)\x01",          # 1
            "t1100 ?????外観帝国貴族邸前(昼)\x01",      # 2
            "t110b ?????外観帝国貴族邸前(夜)\x01",      # 3
            "×t1110 ?????貴族邸内部(昼)\x01",          # 4
            "t111b ?????貴族邸内部(夜)\x01",            # 5
            "t1300 テーマパーク内部(昼)\x01",           # 6
            "t1400 テーマパークお城外観(昼)\x01",       # 7
            "キャンセル\x01",                           # 8
        )
    )
    ...
    Menu(
        ...
        1,
        (
            "T1110 エントランス（昼）\x01",          # 0
            "T1120 オークション会場（昼）\x01",      # 1
            "T1130 １F右翼?小部屋（昼）\x01",        # 2
            "T1140 １F左翼?小部屋（昼）\x01",        # 3
            "T1150 １F温室（昼）\x01",               # 4
            "T1160 ２Fキーア安置室（昼）\x01",       # 5
    ...
    Menu(
        ...
        1,
        (
            "t1300 テーマパーク外観\x01",            # 0
            "t1310 ビーチ\x01",                      # 1
            "t1320 ビーチ受付、更衣室\x01",          # 2
            "t1330 観覧車エリア外観\x01",            # 3
            "t1340 観覧車内部\x01",                  # 4
            "t1350 ホラーハウスエリア外観\x01",      # 5
            "t1370 休憩所\x01",                      # 6
            "t1380 占い館内部\x01",                  # 7
            "t1070 宝石店内部\x01",                  # 8
            "t1390 イベントロッカールーム\x01",      # 9
            "キャンセル\x01",                        # 10
        )
    )
    ...
    Menu(
        ...
        (
            ...
            "t4100 ??????大聖堂：墓地\x01",            # 5
            "t4010 ??????大聖堂内部\x01",              # 6
            "t4020 ??????大聖堂：墓守家内部\x01",      # 7
            "t3510 ??????空港内部\x01",                # 8
            "t6000 警察学校複合施設\x01",              # 9
            "e4700 終章用黒月イベントマップ\x01",      # 10
            "イベント用01\x01",                        # 11
            "イベント用02\x01",                        # 12
            "キャンセル\x01",                          # 13
        )
    ...
    Menu(
        ...
        (
            ...
            "e4200 東クロスベルループ\x01",              # 7
            "e4201 東クロスベルメルカバ墜落\x01",        # 8
            "e4600 星辰の間\x01",                        # 9
    ...
    Menu(
        ...
        1,
        (
            "R4000 森林地帯街道エリア01\x01",        # 0
            "R4010 森林地帯街道エリア02\x01",        # 1
            "R4050 森林地帯訓練エリア①\x01",        # 2
            "R4060 森林地帯訓練エリア②\x01",        # 3
            "R4090 森林地帯訓練ボスエリア\x01",      # 4
            "キャンセル\x01",                        # 5
        )
    )
    ...
    Menu(
        ...
        (
            ...
            "×C0590 ルバーチェ?右F3\x01",             # 1
            "×C0591 ルバーチェ?右F2\x01",             # 2
            "×C0592 ルバーチェ?右F1\x01",             # 3
            "C0593 ルバーチェ?右B1\x01",               # 4
            "×C0594 ルバーチェ?左F1\x01",             # 5
            "×C0595 ルバーチェ?左F2\x01",             # 6
            "×C0596 ルバーチェ?左F3\x01",             # 7
            "C0597 ルバーチェ?地下\x01",               # 8
            "キャンセル\x01",                          # 9
        )
    )
    ...
    Menu(
        ...
        1,
        (
            "M4000 アルタイルロッジ外観\x01",      # 0
            "M4010 アルタイル攻略01\x01",          # 1
            "M4020 アルタイル攻略02\x01",          # 2
            "M4040 アルタイル中継\x01",            # 3
            "M4050 アルタイル攻略03\x01",          # 4
            "M4060 アルタイル攻略04\x01",          # 5
            "M4090 アルタイルボス\x01",            # 6
            "キャンセル\x01",                      # 7
        )
    )
    ...
    RemoveCraft(0x1, 0x7B)
    RemoveCraft(0x1, 0x7C)
    ...
    RemoveCraft(0x1, 0x87)
    Return()

    # Function_46_481C end

    def Function_47_4959(): pass
    ...
    RemoveCraft(0x2, 0x7B)
    RemoveCraft(0x2, 0x7C)
    ...
    RemoveCraft(0x2, 0x87)
    Return()

    # Function_47_4959 end

    def Function_48_4A96(): pass
    ...
    RemoveCraft(0x3, 0x7B)
    RemoveCraft(0x3, 0x7C)
    ...
    RemoveCraft(0x3, 0x87)
    Return()

    # Function_48_4A96 end

    def Function_49_4BD3(): pass
    ...
    RemoveCraft(0x6, 0xE)
    RemoveCraft(0x6, 0x19)
    RemoveCraft(0x6, 0x22)
    RemoveCraft(0x6, 0x2C)
    RemoveCraft(0x6, 0x34)
    RemoveCraft(0x6, 0x3F)
    RemoveCraft(0x6, 0x52)
    RemoveCraft(0x6, 0x5A)
    RemoveCraft(0x6, 0x64)
    ...
    Menu(
        ...
        (
            ...
            "▽１章１日目③：セルゲイからの連\x01",      # 1
            "▽１章１日目③：新型導力車を受け\x01",      # 2
            "▽１章１日目③：シスター?リース\x01",       # 3
            "▽１章１日目③：年長クラスの授業\x01",      # 4
            "▼１章２日目④：２日目を開始した\x01",      # 5
            "▽１章２日目⑤：マインツに到着し\x01",      # 6
            "▽１章２日目⑤：ランディがクリム\x01",      # 7
            "キャンセル\x01",                            # 8
        )
    )
    ...
    Menu(
        ...
        (
            ...
            "▽２章２日目②：ヨナから連絡がき\x01",      # 3
            "▼２章３日目③：３日目を開始した\x01",      # 4
            "▽２章３日目③：オルキスタワー案\x01",      # 5
            "▽２章３日目④：巡回を開始した\x01",        # 6
            "▽２章３日目④：休憩時間になった\x01",      # 7
            "▼\x01",                                    # 8
            "キャンセル\x01",                            # 9
        )
    )
    ...
    Menu(
        ...
        1,
        (
            "▽幕間１日目①：水上バスでの自由\x01",      # 0
            "▼幕間１日目①：ホテルの部屋に到\x01",      # 1
            "▽幕間１日目①：ビーチでの自由行\x01",      # 2
            "▼幕間１日目②：キーアを探しに行\x01",      # 3
            "▼幕間１日目②：キーアを発見した\x01",      # 4
            "▽幕間１日目②：テーマパークに入\x01",      # 5
            "▼幕間１日目③：パートナーと合流\x01",      # 6
            "▽幕間１日目③：深夜に目が覚めた\x01",      # 7
            "▽幕間１日目③：夜、キーアを探し\x01",      # 8
            "キャンセル\x01",                            # 9
        )
    )
    ...
    Menu(
        ...
        (
            ...
            "▽３章１日目①：イアン弁護士と遭\x01",      # 1
            "▼３章２日目②：人形工房を訪ねる\x01",      # 2
            "▼３章２日目②：脱線事故の連絡が\x01",      # 3
            "▼３章３日目③：ミシェルからの連\x01",      # 4
            "▼３章４日目④：ランディの捜索を\x01",      # 5
            "キャンセル\x01",                            # 6
        )
    )
    ...
    Menu(
        ...
        1,
        (
            "▼終章１日目①：ウルスラ間道の中\x01",      # 0
            "▼終章１日目⑦：ヨルグから連絡が\x01",      # 1
            "▽終章２日目⑧：ダドリーと合流し\x01",      # 2
